...
struct bitmap *bitmap_expand(struct bitmap *bitmap, int size){
	size_t new_size = bitmap_size(bitmap) + size;
	struct bitmap *temp = bitmap_create(new_size);
	for(int i = 0; i<bitmap_size(bitmap); i++) bitmap_set(temp, i, bitmap_test(bitmap, i));
